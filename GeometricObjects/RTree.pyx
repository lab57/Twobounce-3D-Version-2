...
cdef class BoundingBox:
    cdef public list[float] min_point, max_point
    def __cinit__(self, min_point: list[float], max_point: list[float]):
    ...
    cpdef float compute_t(self, plane_coord, origin_coord, direction_coord):
    ...
    cpdef bint intersects(self, origin: Vector, direction: Vector):
        cdef float t1, t2, t3, t4, t5, t6, tmin, tmax

        t1 = self.compute_t(self.min_point[0], origin[0], direction[0])
        t2 = self.compute_t(self.max_point[0], origin[0], direction[0])
        t3 = self.compute_t(self.min_point[1], origin[1], direction[1])
        t4 = self.compute_t(self.max_point[1], origin[1], direction[1])
        t5 = self.compute_t(self.min_point[2], origin[2], direction[2])
        t6 = self.compute_t(self.max_point[2], origin[2], direction[2])
...
class RTreeNode:
    ...
    cdef BoundingBox bounding_box
    cdef list[Triangle], 
    def __cinit__(self, bounding_box: BoundingBox, triangles: list[Triangle] | None =None, children: list[RTreeNode] | None =None):
        """R-Tree node.
